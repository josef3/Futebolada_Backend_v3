...
describe('GET /years/:id/weeks', () => {
	test(`It should return 200 & years's weeks info`, async () => {
...
describe('GET /years/:id/poty', () => {
	test(`It should return 200 & years's poty info`, async () => {
		const res = await request(app).get(`${API_URL}/1/poty`);

		expect(res.status).toBe(200);

		expect(Object.keys(res.body).length).toBe(7);
		expect(res.body).toHaveProperty('id_poty');
		expect(res.body).toHaveProperty('id_year');
		expect(res.body.id_year).toBe(1);
		...
		expect(res.body).toHaveProperty('no_bg_avatar_url');
	});

	test(`It should return 200 & empty object if year exists but doesn't have a poty yet`, async () => {
		const res = await request(app).get(`${API_URL}/2/poty`);
